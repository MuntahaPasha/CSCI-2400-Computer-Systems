...
1) Computer Systems Textbook
2) For guidance with Coalesce, Realloc, and some other little things: https://github.com/timkaboya/malloc/blob/master/mm.c
3) Guidance w/ Place: https://github.com/HarshTrivedi/malloc/blob/master/mm.c
4) Understanding Epi/Prologue Headers & Footers: https://ocw.mit.edu/courses/electrical-engineering-and-computer-science/6-087-practical-programming-in-c-january-iap-2010/lecture-notes/MIT6_087IAP10_lec11.pdf
*/
...
//WE IMPLEMENTED EXPLICIT (We check only for free blocks)
//Implicit - Check every block
//Segregated - 
...
#define ALIGNMENT 8 //signle word(4) or double word (8) Allocation has to start at 8. It aligns it. You can't have allocation starting at bit 3.
#define MINIMUM   24
...
// sand initializes them to create empty free list
// return -1 on error, 0 on success
int mm_init(void)
{
    //mem_sbrk extends the heap by incr bytes and return the start address of new area
    // in this case just check, if sbrk fail, ran out of memory
    
    /*create the initial empty heap(free list) */
    //Sbrk = It gets the start of the memory address (of virtual memory) and it also extends it to twice of the minimum.
    //heaplistp is in charge of keeping track of allocated blocks.
    if((heap_listp = mem_sbrk(2*MINIMUM))== (void *)-1)
        return -1;
    //Clearing the memory.
    PUT(heap_listp, 0);  //Alignment padding
    //Establishing the header, footer, and the padding. 
    ...
    //128/4=32. Expand by 32 bytes. Chunksizeis 1<<8, which is 1000 0000 and decimal is 128.
...
static void *extend_heap(uint32_t words)
{
    ...
    //heap space from the memeory system
    //if word is odd, add 1 to it, if not leave it. They want to maintain word size.
    size = (words % 2)? (words + 1) * WSIZE : words * WSIZE;
    //boundary check.
    if (size < MINIMUM)
        size = MINIMUM;
    //return address of heaplistp, will compress to 32 bytes.
    if((long)(bp = mem_sbrk(size)) == -1)
        return NULL;
    
    //the heap begins on a double word aligned boundary
    //every call to extend_heap return a block whose size is an intgral number
    //of double word
    ...
    //prologue header contains the size of the current block
    //epilogue header is an allocated block of size 0.
    PUT(HDRP(NEXT_BLKP(bp)), PACK(0, 1)); //new epilogue header
    
    //in the case that previous heap was terminated by a free block
    //we call Coalesece function to merge the two free block
...
static void *find_fit(uint32_t asize)
{
    ...
    //free list is a list of all possible free blocks.
    for(bp = free_listp; GET_SIZE(HDRP(bp))> 0; bp = NEXT_BLKP(bp)){
        //if size you need less than equal to free list block size, then
        //return block pointer to that space.
...
{
    //if pointer is null, then just return.
    if(bp == 0)
        return;
    
    //get the size of the block given
    uint32_t size = GET_SIZE(HDRP(bp));

    //if memory manager has not been intialized, then it initializes it.
    if(heap_listp == 0)
    {
        mm_init();
    }
    ...
    //calls it to merge free blocks together
    coalesce(bp);
...
static void *coalesce(void *bp)
{
   ...
   //In the case where we only have one block, set the prev block to 1.
    if(PREV_BLKP(bp) == bp)
    {
        prev_alloc = 1;
    }
    ...
    //case1 Just append new block to list
    ...
    */
    if(prev_alloc && !next_alloc){
        //gets the size of next block and adds it to the size of current block. 
        //Next block hasn't been allocated yet, so it will combine the two blocks and thats why
        //we are adding the size.
        size += GET_SIZE(HDRP(NEXT_BLKP(bp)));
        RemoveFreeBlock(NEXT_BLKP(bp)); //remove next block
        //Since we merged the sizes, it updates the header and footer of the new free block.
    ...
    */
    ...
    {
        //It will combine current and previous together.
        size += GET_SIZE(HDRP(PREV_BLKP(bp)));
        RemoveFreeBlock(PREV_BLKP(bp)); //remove previous block
        PUT(FTRP(bp), PACK(size, 0));
        PUT(HDRP(PREV_BLKP(bp)), PACK(size, 0));
        //sets the block pointer to the previous block.
        bp = PREV_BLKP(bp);
    }
    ...
    */
        ...
        //gets sizes of previous and next, adds them.
        ...
        //sets block pointer to previous, a.k.a first block.
        bp = PREV_BLKP(bp);
    }
...
void *mm_malloc(uint32_t size)
{
    ...
    //aligning to even bytes, word size, and any other header and footer bytes.
    ...
    //variable to hold how many bytes you need to extend heap by.
    extend_size = MAX(asize, CHUNKSIZE);
    //calls extend heap to do the extending.
...
static void place(void *bp, uint32_t asize)
{
    /*| alloc(8) | --> |   free(16)   |
    |alloc(8) | free(8) |
    This free then goes into freelist to be used for something else.
    */

    //csize is the size of the entire block.
    size_t csize = GET_SIZE(HDRP(bp));
    
    //Split the free block off, and use it in freelist.
    ...
    //if we don't have any free memory in the block, remove it from the free list.
    else
    {
...
// if you need to enlarge the amount of data you have, you can reallocate that
// data into pre-existing memory into a different size.
//*ptr contains address to existing memory
//size is the number of bytes you want to reallocate.
void *mm_realloc(void *ptr, uint32_t size)
{
    
    void *newp;
    //copy the numner of bytes you currently have.
    uint32_t copySize;
    
    //allocates memory of the new size.
    newp = mm_malloc(size);
    ...
    //reduce copysize if less bytes are needed.
    if (size < copySize) {
        copySize = size;
    }

    //copy memory from existing byte into the new block.
 ...
 */
//Adds free blocks to the free list.
static void InsertFreeBlock(void *bp)
{
...
static void RemoveFreeBlock(void *bp)
{
    ...
    /* Case 1 */
    //It has nothing that we need to free. If previous is null and next is null then we have nothing we want to free, so free list is set to 0.
