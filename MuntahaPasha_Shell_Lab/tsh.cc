...
//once in the calling process(parent), and once in newly create childe process
...
void eval(char *cmdline)
{
    ...
    sigset_t mask;  
    sigemptyset(&mask); //initializes set to the empty set
    sigaddset(&mask, SIGCHLD); //set the mask to the SIGCHID
    ...
    if(!builtin_cmd(argv)){
        ...
        if ((pid = Fork()) == 0){
            ...
            //execve function load and run the executable object file with
            //list argv and the enviroment variable list envp
            
            
            // test if command is found
            //execve return to the calling program only if there is an error
            //example can't find the argv name
            //
...
void do_bgfg(char **argv)
{
    ...
    if (isdigit(argv[1][0])) {
        ...
        //if there is no process id, just return
...
// Signal handlers
//
// signal is a small message that notified a process that an even to some type hase occured in the system
...
//
void sigchld_handler(int sig) //a child process has stopped or terminate
{                           //kernel sent a SIGCHLD signal to the parent
    pid_t wpid;
    int status;
    //Will reap all zombie children //W|W doesnt hang for stopped jobs
    //WNOHANG = don't blocck waiting
    //WUNTRACED = report status of stopped children
    //waitpid function use to reap zombie children in no particular order
    ...
    //waitpid function terminated normally and print the error message otherwise.
    if (errno != ECHILD){
...
void sigint_handler(int sig)
{
    ...
    if (pid != 0){
        kill(-pid, SIGINT); // SIGINT = interrrupt from keyboard
    }
}
...
void sigtstp_handler(int sig)
{
    ...
    if (pid != 0){
        kill(-pid, SIGTSTP);  //SIGTSTP = stop signal from terminal
    }
